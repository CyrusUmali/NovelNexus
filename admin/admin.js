...
function checkUserPlan() {
    fetch('../db/php/dashboard/checkUserplan-api.php')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                console.log('Loan statuses updated successfully.');
...
function fetchBooks(searchQuery = "") {
    ...
    axios
        .get(
            `../db/php/Fetch_books-api.php?search=${encodeURIComponent(
                searchQuery
...
function addNewBookClick() {
    ...
    const originalButtonText = addButton.innerHTML; // Save the original button content
    ...
    const accesslvl = document.getElementById('accessLvl').value;
    ...
    // Validation passed: show loading indicator
    addButton.disabled = true;
    addButton.innerHTML = '<span class="spinner"></span> Uploading...'; // Add loading indicator
    ...
    uploadImageToCloudinary(bookCover).then(imageUrl => {
        ...
        axios.post('../db/php/addBook-api.php', {
            ...
            about: aboutBook,
            accesslvl: accesslvl, // Include the access level
        })
            .then(response => {
            ...
            .finally(() => {
                ...
                addButton.innerHTML = originalButtonText; // Restore original button text
            });
    }).catch(error => {
        console.error('Error uploading image:', error);
                    ...
                    datasets: [
                        {
                            ...
                            borderColor: borderColor, // Colors for the wrapping border
...
let bookAvailabilityChartInstance = null; // Store the chart instance
...
function initializeBookAvailabilityChart() {
    ...
    axios.get('../db/php/dashboard/bookAvailability-api.php')
        .then((response) => {
            ...
            // Select the chart container
            const bookAvailabilityChartElement = document.querySelector('.book-availability-chart');

            // Destroy the existing chart if it exists
            if (bookAvailabilityChartInstance) {
                bookAvailabilityChartInstance.destroy();
            }

            // Render the new chart and store its instance
            bookAvailabilityChartInstance = new Chart(bookAvailabilityChartElement, {
                type: 'doughnut',
                data: {
            ...
            // Display the total availability and borrowed books
