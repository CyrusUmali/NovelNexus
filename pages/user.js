...
//             // Group books by title or ISBN
//             const groupedBooks = books.reduce((groups, book) => {
//                 const key = book.title;  // or use book.isbn to group by ISBN
//                 if (!groups[key]) {
//                     groups[key] = [];
//                 }
...
//             // Loop over the grouped books
//             for (const key in groupedBooks) {
//                 const bookGroup = groupedBooks[key];
//                 const mainBook = bookGroup[0]; // The first book in the group

//                 // Create the main row first
//                 const row = document.createElement("tr");

//                 // Create a unique identifier for the subgroup based on the group
//                 const subgroupId = `subgroup-${mainBook.id}`;

//                 // Check if the book has multiple copies or not
//                 if (bookGroup.length > 1) {
//                     // Assign the onclick handler for the main row if there are multiple copies
//                     row.onclick = () => {
//                         const subgroup = document.getElementById(subgroupId);
//                         if (subgroup) {
//                             // Toggle a class on the specific subgroup to show/hide it
//                             subgroup.classList.toggle('show');
//                         }
//                     };
//                 } else {
//                     // Normal onclick for books with a single copy
//                     row.onclick = () => viewBookClick(mainBook.id);  // Assuming `viewBookClick` is a function that handles a single book click
//                 }

//                 // Set up the content for the main book row
//                 row.innerHTML = `
//         <td>
//             <div class="tdImg">
//                 <img src="${mainBook.book_cover}" alt="Book Cover" />
//                 <span class="span1">${mainBook.title}</span>
//             </div>
//         </td> 
//         <td>
//             <div class="wrapper">
//                 <span class="span1">${mainBook.author}</span>
//             </div>
//         </td>
//         <td>
//             ${mainBook.average_rating ?
//                         `<span class="rating">${mainBook.average_rating.toFixed(1)}</span>` :
//                         `<span class="rating">--</span>`}
//         </td>
//         <td>
//             <div class="book-info">
//                 <span class="title">${mainBook.category_names}</span>
//             </div>
//         </td>
//        <td>
//     <span class="green">
//         Available: ${bookGroup.filter(book => book.Availability === 'Available').length}
//     </span>
//     <br>
//     <span class="red">
//         Unavailable: ${bookGroup.filter(book => book.Availability === 'Loaned').length}
//     </span>
// </td>

//         <td>
//             ${mainBook.in_shelf ?
//                         `<i class="blue shelved" onclick="event.stopPropagation(); removeFromShelf(${mainBook.id},this )">
//                     <i class='bx bx-cabinet nav__icon'></i>  
//                 </i>` :
//                         `<i class="blue" onclick="event.stopPropagation(); addToShelf(this , ${mainBook.id} )">
//                     <i class='bx bx-cabinet nav__icon'></i> 
//                 </i>`}
//         </td>
//     `;
...
//                 // Add sub-rows for additional copies if more than one book in the group
//                 if (bookGroup.length > 1) {
//                     const subRow = document.createElement("tr");
//                     subRow.classList.add("subGroup");
//                     subRow.id = subgroupId;  // Assign the unique ID to the subgroup

//                     subRow.innerHTML = `
//             <td colspan="6">
//                 <div class="nested-book-wrapper">
//                     ${bookGroup.map(book => `
//                         <div class="item" onclick="viewBookClick(${book.id})"    >
//                             <div class="bookInfo">
//                                 <img src="${book.book_cover}" alt="Book Cover" />
//                                 <span class="span1">${book.title}</span>
//                             </div>
//                             <span class="${book.Availability === 'Loaned' ? 'red' : 'green'}">${book.Availability}</span>
//                         </div>
//                     `).join('')}
//                 </div>
//             </td>
...
//                     // Initially hide the subgroup (if needed)
//                     subRow.classList.add('hide'); // You can use this class to hide the subgroup by default.

//                     // Add the sub-row under the main row
...
// Attach event listeners to filter and sort icons
document.querySelectorAll('.books-table thead i').forEach(icon => {
    icon.addEventListener('click', function (event) {
        // Prevent default row sorting when clicked
        event.stopPropagation();

        // Remove 'navActive' from all icons
        document.querySelectorAll('.books-table thead .bx.bx-sort.navActive').forEach(activeIcon => {
            activeIcon.classList.remove('navActive');
        });

        // Add 'navActive' to the clicked icon
...
let currentFilter = ""; // Default: Show both Available and Loaned

function toggleFilter() {
    const filterIcon = document.querySelector(".availability-filter i");

    if (currentFilter === "Available") {
        currentFilter = ""; // Reset filter to show all
        filterIcon.classList.remove("navActive");
    } else {
        currentFilter = "Available"; // Show only available books
        filterIcon.classList.add("navActive");
    }
...
function toggleSort(column) {
    if (currentSort === column) {
        ...
        currentSort = column;
        currentDirection = "ASC"; // Default to ascending order when changing column
    }
...
function fetchBooks(searchQuery = "") {
    ...
    axios
        .get(
            `./db/php/userFetch_books-api.php?search=${encodeURIComponent(
                searchQuery
            )}&category=${encodeURIComponent(
                categoryId
            )}&sort=${encodeURIComponent(currentSort)}&direction=${encodeURIComponent(
                currentDirection
            ...
            if (data.status === 'success') {
                const notificationsHtml = data.notifications.map(notification => {
                    if (notification.message !== null) {
                        ...
                        // Determine the message based on daysLeft
                        if (daysLeft > 0) {
                            message = `Your book '${notification.book_title}' is due in ${daysLeft} days.`;
                        } else if (daysLeft === 0) {
                            message = `Your book '${notification.book_title}' is due today.`;
                        } else {
